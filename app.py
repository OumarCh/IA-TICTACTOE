...
class TicTacToe(TwoPlayersGame):
    ...
    #WIN_LINES = [[1,2,3,4],[2,3,4,5],[1,6,11,16],
                                       ...
                                       #[22,18,14,10]
                                      #]
    WIN_LINES = [[1,2,3],[4,5,6],[7,8,9], # horiz.
    ...
    def show(self):
        print ('\n' + '\n'.join([
            ' '.join(
                [['.', 'O', 'X'][self.board[self.grid * j + i]] for i in range(self.grid)]
            )
            for j in range(self.grid)
        ]))

    def spot_string(self, i, j):
        return ["_", "O", "X"][self.board[self.grid * j + i]]
